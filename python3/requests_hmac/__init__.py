...
class HTTPSignatureAuth(requests.auth.AuthBase):
    ...
    def __call__(self, request):
        request.headers["Authorization"] = "hmac " + self.create_signature_string(request)
        return request
    ...
    def verify(self, request, key_resolver, scheme="Authorization"):
        if scheme == "Authorization":
            ...
            assert "hmac" in request.headers, "No Signature header found"
        else:
        ...
        for field in "username", "algorithm", "signature":
        ...
        key = key_resolver(key_id=sig_struct["username"], algorithm=sig_struct["algorithm"])
...
class HTTPSignatureHeaderAuth(HTTPSignatureAuth):
    ...
    def __call__(self, request):
        request.headers["hmac"] = self.create_signature_string(request)
        return request
